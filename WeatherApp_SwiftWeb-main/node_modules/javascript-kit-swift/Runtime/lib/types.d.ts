...
export declare type ref = number;
export declare type pointer = number;
export declare type bool = number;
export declare type JavaScriptValueKind = number;
export declare type JavaScriptValueKindAndFlags = number;
export interface ExportedFunctions {
    ...
    swjs_call_host_function(host_func_id: number, argv: pointer, argc: number, callback_func_ref: ref): bool;
    swjs_free_host_function(host_func_id: number): void;
}
export interface ImportedFunctions {
    swjs_set_prop(ref: number, name: number, kind: JSValue.Kind, payload1: number, payload2: number): void;
    swjs_get_prop(ref: number, name: number, payload1_ptr: pointer, payload2_ptr: pointer): JavaScriptValueKind;
    swjs_set_subscript(ref: number, index: number, kind: JSValue.Kind, payload1: number, payload2: number): void;
...
export declare const enum LibraryFeatures {
    WeakRefs = 1
}
export declare type TypedArray = Int8ArrayConstructor | Uint8ArrayConstructor | Int16ArrayConstructor | Uint16ArrayConstructor | Int32ArrayConstructor | Uint32ArrayConstructor | BigInt64ArrayConstructor | BigUint64ArrayConstructor | Float32ArrayConstructor | Float64ArrayConstructor;
export declare function assertNever(x: never, message: string): void;
